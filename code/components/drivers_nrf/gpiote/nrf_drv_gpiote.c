...
extern void Tsmxx_Irq_Disable(void);
extern void SysBasetimer_Start(uint16 time);
extern void Carddet_timer_start(uint16 time);


void GPIOTE_IRQHandler(void)
{
    if(NRF_GPIOTE->EVENTS_PORT&0x00000001)
    {
      NRF_GPIOTE->EVENTS_PORT = 0; 
#if (defined TOUCH_ENABLE) && (TOUCH_ENABLE == STD_TRUE)
        if( TouchIntatGetData() ==0 )
         {
            TouchTimer64ms = TOUCH_TIME_OUT_DATA;
            //if((Sys_CheckCardSleepDet() == 0) && (Sys_State == SYS_SLEEP))
            //{
