...
{
//             DI();
       AES_ECB_decrypt(input, Sys_PataCfg.Aeskey, output, 16);
//         EI();
}

void AES_Encrypt(const uint8_t* input, uint8_t *output)
{
    //   AES_ECB_encrypt(input, Aes_KeyTable, output, 16);
    nrf_ecb_hal_data_t    nrf_ecb_dat; 

    memcpy(nrf_ecb_dat.key,Sys_PataCfg.Aeskey,16);
    memcpy(nrf_ecb_dat.cleartext,input,16);
    sd_ecb_block_encrypt(&nrf_ecb_dat);
    memcpy(output,nrf_ecb_dat.ciphertext,16);
}
