...
const unsigned char ip_permtab[]  = {
         8,  8,                                         /* 64 bit -> 64 bit */
...
const unsigned char inv_ip_permtab[]  = {
         8, 8,                                          /* 64 bit -> 64 bit */
...
const unsigned char pc1_permtab[]  = {
         8,  7,                                         /* 64 bit -> 56 bit*/
        57, 49, 41, 33, 25, 17,  9,
         1, 58, 50, 42, 34, 26, 18,
        10,  2, 59, 51, 43, 35, 27,
...
const unsigned char pc2_permtab[]  = {
         7,      6,                                     /* 56 bit -> 48 bit */
        14, 17, 11, 24,  1,  5,
         3, 28, 15,  6, 21, 10,
        23, 19, 12,  4, 26,  8,
...
const unsigned char splitin6bitword_permtab[]  = {
         8,  8,                                         /* 64 bit -> 64 bit */
        64, 64,  1,  6,  2,  3,  4,  5, 
...
const unsigned char shiftkey_permtab[]  = {
         7,  7,                                         /* 56 bit -> 56 bit */
         2,  3,  4,  5,  6,  7,  8,  9,
...
const unsigned char shiftkeyinv_permtab[]  = {
         7,  7,
        28,  1,  2,  3,  4,  5,  6,  7,
...
void DES_DES(void)
{
	unsigned char ar_iv[8] = { 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x01 };
	memcpy(iv,ar_iv,8);
	memcpy(&IVC,ar_iv,8);
 	arr_pad[0] = 0x01;
	arr_pad[1] = 0x02;
...
unsigned char pgm_read_byte (const uint8_t *abc)
{
    return *abc;
}
...
void DES_permute(const unsigned char *ptable, const unsigned char *in, unsigned char *out)
	{
        unsigned char ob; /* in-bytes and out-bytes */
        unsigned char byte, bit; /* counter for bit and byte */
...
void DES_shiftkey(unsigned char *key){
        unsigned char k[7];
        memcpy(k, key, 7);
        DES_permute((unsigned char*)shiftkey_permtab, k, key);    
}
...
void DES_shiftkey_inv(unsigned char *key){
        unsigned char k[7];
        memcpy(k, key, 7);
        DES_permute((unsigned char*)shiftkeyinv_permtab, k, key);
...
uint64_t DES_splitin6bitwords(uint64_t a){
        ...
        DES_permute((unsigned char*)splitin6bitword_permtab, (unsigned char*)&a, (unsigned char*)&ret);       
        return ret;
}
...
unsigned long DES_des_f(unsigned long r, unsigned char* kr){
        unsigned char i;
        unsigned long t=0,ret;
        uint64_t data;
        unsigned char *sbp; /* sboxpointer */ 
        DES_permute((unsigned char*)e_permtab, (unsigned char*)&r, (unsigned char*)&data);
        for(i=0; i<7; ++i)
                ((unsigned char*)&data)[i] ^= kr[i];
        
        /* Sbox substitution */
        data = DES_splitin6bitwords(data);
        sbp=(unsigned char*)sbox;
        for(i=0; i<8; ++i){
                unsigned char x;
                x = DES_substitute(((unsigned char*)&data)[i], sbp);
                t <<= 4;
                t |= x;
                sbp += 32;
        }
				
        DES_changeendian32(&t);
                
        DES_permute((uint8_t*)p_permtab,(uint8_t*)&t, (uint8_t*)&ret);
...
#define L (data.v32[0])
        ...
        DES_permute((uint8_t*)ip_permtab, (uint8_t*)in, data.v8);
        DES_permute((uint8_t*)pc1_permtab, (const uint8_t*)key, k);
        for(i=0; i<8; ++i){
                DES_shiftkey(k);
                if(ROTTABLE&((1<<((i<<1)+0))) )
                        DES_shiftkey(k);
                DES_permute((uint8_t*)pc2_permtab, k, kr);
                L ^= DES_des_f(R, kr);
        ...
        L ^= R;
        R ^= L;
        DES_permute((uint8_t*)inv_ip_permtab, data.v8, (uint8_t*)out);
}
...
void DES_tripleEncrypt(void* out, void* in, const void* key){
        DES_encrypt(out,  in, (uint8_t*)key + 0);
        DES_decrypt(out, out, (uint8_t*)key + 8);
        DES_encrypt(out, out, (uint8_t*)key +16);
}

/******************************************************************************/

void DES_tripleDecrypt(void* out, void* in, const uint8_t* key){
        DES_decrypt(out,  in, (uint8_t*)key +16);
        DES_encrypt(out, out, (uint8_t*)key + 8);
        DES_decrypt(out, out, (uint8_t*)key + 0);
}
...
bool DES_CheckPad(unsigned char* in,int lsize){
	if (in[lsize-1] <= 0x08){	
...
void DES_tdesCbcEncipher(unsigned char* in,unsigned char* out)
{
  ...
  for (int i = 0; i < size; i += 8)
  {
    ...
    DES_tripleEncrypt(out + i, in + i, key);
  }
...
void DES_tdesCbcDecipher(unsigned char* in,unsigned char* out)
{
  ...
  for (int i = 0; i < size; i += 8)
  {
    DES_tripleDecrypt(out + i, in + i, key);
...
void DES_do_3des_encrypt(unsigned char *plain,int size_p,unsigned char *cipher,const void *key, bool inc){
	...
	DES_calc_size_n_pad(size_p);
	unsigned char plain_p[DES_get_size()];
	DES_padPlaintext(plain,plain_p);
	DES_change_key (key);
	DES_tdesCbcEncipher(plain_p,cipher);
}
...
void DES_do_3des_decrypt(unsigned char *cipher,int size_c,unsigned char *plain,const void *key, unsigned long ivl){
	...
	DES_change_key (key);
	DES_set_IV(ivl);
	DES_tdesCbcDecipher(cipher,plain);
}
...
void desExample(void)
{
  ...
  unsigned char out[8] = {0x00};
  unsigned char in[8]  = {0x27, 0xA1, 0x83, 0xC3, 0x00, 0x00, 0x00, 0x00};
  unsigned char key[8] = {0x11, 0x22, 0x33, 0x44, 0x11, 0x22, 0x33, 0x44};
...
{
  unsigned char out[8] = {0x00};
  unsigned char in[8]  = {0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08};
  unsigned char key[24]={
